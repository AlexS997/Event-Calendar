    ...
    public class ClsTableSchema
    {
        ...
        public void Apri(string connectionString, string tableName)
        {
            using (SqlConnection Cn = new SqlConnection(connectionString))
            {
                ...
                /* Creates a simple SQL query that selects all columns in the table, 
                 * but does not extract data (WHERE 1 = 0 is a condition that will not return any line, but will only return the scheme). */
